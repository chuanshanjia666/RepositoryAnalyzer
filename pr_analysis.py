    ...
    def __init__(self, token: Optional[str] = None, config: PRAnalyzerConfig = None):
        ...
        try:
            if self.token:
                ...
                print("  未配置GitHub token，将使用无认证模式（API配额受限）")
    ...
    def _set_user_agent(self):
        ...
        try:
            ...
            try:
                ...
                print("  无法设置User-Agent，可能影响API访问（非关键错误）")
...
class PRDataFetcher:
    ...
    def _get_cache_dir(self) -> str:
        ...
        try:
            ...
            print(f"  使用系统临时目录失败，改用当前目录: {str(e)}")
            return ".cache"
    ...
    def _handle_api_rate_limit(self, exception):
        ...
        if isinstance(exception, GithubException) and exception.status == 403:
            try:
                ...
                print(f"  动态计算限流等待时间失败，使用默认策略: {str(e)}")
                wait_time = self.config.retry_wait_base * (self.current_attempt + 1)
...
"""
    ...
    def _generate_author_analysis(self) -> str:
        ...
        try:
            ...
            table_rows = []
            for author, count in authors[:self.config.max_authors_display]:
    ...
    def _generate_label_analysis(self) -> str:
        ...
        try:
            ...
            table_rows = []
            for label, count in labels[:self.config.max_authors_display]:
                percentage = (count / total) * 100
...
class PRAnalyzer:
    ...
    def run_analysis(self, save_reports: bool = True) -> Dict:
        ...
        try:
            ...
            self.metrics['days_threshold'] = self.config.zombie_threshold_days
            if save_reports:
                print("3. 生成可视化报告")
